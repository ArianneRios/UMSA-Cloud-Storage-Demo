...
export class UsuariosService implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.crearFuncionSql();
    await this.crearUsuarioDemo();
  }

  async crearFuncionSql() {
    const query = `
    ...
    `;
    try {
      await this.usuarioRepository.query(query);
      console.log('Función SQL fn_registrar_archivo_subido sincronizada correctamente');
    } catch (error) {
      console.error('Error al crear la función SQL en PostgreSQL:', error);
    }
  }

  async crearUsuarioDemo() {
    const correoDemo = 'demo@cloud.com';
    const existe = await this.buscarPorCorreo(correoDemo);
    ...
    if (!existe) {
      ...
      const usuarioDemo = this.usuarioRepository.create({
        nombre: 'Usuario Demo',
        correo: correoDemo,
        passwordHash,
        rol: 'ESTUDIANTE',
      ...
      await this.usuarioRepository.save(usuarioDemo);
